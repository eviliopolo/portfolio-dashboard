...
export async function readExcelFile(file: File | ArrayBuffer | string): Promise<ExcelData> {
  ...
  sheetNames.forEach((sheetName) => {
    ...
    // Usar raw: true para mantener los valores originales y procesar manualmente
    ...
    switch (sheetName) {
      case 'Dashboard_Resumen':
        ...
        break;
      case 'Proyectos':
        // Convertir fechas y validar números en la hoja de Proyectos
        data.proyectos = (jsonData as any[]).map(item => {
          // Mapear nombres de columnas del Excel a nombres internos
          const processed: any = { ...item };
          
          // Mapear Fecha_Inicio, Fecha_Fin, Fecha_Entrega a Inicio, Fin, Entrega
          ...
          // Si las fechas ya vienen en formato yyyy-MM-dd, mantenerlas así
          // Solo procesar si necesitan conversión (números seriales de Excel)
          ...
          ['Inicio', 'Fin', 'Entrega'].forEach(field => {
            if (field in processedWithDates) {
              ...
              // Si ya es un string en formato yyyy-MM-dd, mantenerlo
              if (typeof value === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(value.trim())) {
                // Ya está en formato correcto, no hacer nada
                return;
              }
              // Si es un número o otro formato, convertir
          ...
          // Validar Tareas
          if ('Tareas' in processedWithDates) {
            const tareas = processedWithDates.Tareas;
            if (typeof tareas === 'string') {
              const parsed = parseInt(tareas.replace(/[^\d]/g, ''));
              processedWithDates.Tareas = isNaN(parsed) ? 0 : parsed;
            } else if (typeof tareas === 'number') {
              processedWithDates.Tareas = (tareas >= 0 && tareas < 100000) ? tareas : 0;
            } else {
              processedWithDates.Tareas = 0;
            }
          }
          // Validar Recursos
          if ('Recursos' in processedWithDates) {
            const recursos = processedWithDates.Recursos;
            if (typeof recursos === 'string') {
              const parsed = parseInt(recursos.replace(/[^\d]/g, ''));
              processedWithDates.Recursos = isNaN(parsed) ? 0 : parsed;
            } else if (typeof recursos === 'number') {
              processedWithDates.Recursos = (recursos >= 0 && recursos < 10000) ? recursos : 0;
            } else {
              processedWithDates.Recursos = 0;
            }
          }
      ...
      case 'Solapamientos':
        ...
        break;
      case 'Timeline':
        // Convertir fechas en la hoja de Timeline
